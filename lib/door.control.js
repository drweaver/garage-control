...
EventBus.on('garage.open', function(response) {
    if( status == 'Closed' ) {
        exports.operate(response);
    } else {
        response('Open request ignored, door not in Closed state');
    }
});

EventBus.on('garage.close', function(response) {
    if( status == 'Opened' ) {
        exports.operate(response);
    } else {
        response('Close request ignored, door not in Opened state');
    }
});
